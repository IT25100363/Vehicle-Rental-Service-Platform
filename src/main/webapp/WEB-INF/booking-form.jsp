...
<body>
    <div class="form-container">
        <h2>${booking != null ? 'Edit Booking' : 'New Booking'}</h2>
        <form action="bookings" method="post">
            ...
            <div class="form-group"><label>User ID</label><input type="text" name="userId" value="${booking.userId}" required></div>
            <div class="form-group"><label>Vehicle ID</label><input type="text" name="vehicleId" value="${booking.vehicleId}" required></div>
            <div class="form-group"><label>Start Date</label><input type="date" name="startDate" value="${booking.startDate}" required></div>
            <div class="form-group"><label>End Date</label><input type="date" name="endDate" value="${booking.endDate}" required></div>
            <div class="form-group">
                <label>Status</label>
                <select name="status">
                    <option value="Pending" ${booking.status == 'Pending' ? 'selected' : ''}>Pending</option>
                    <option value="Confirmed" ${booking.status == 'Confirmed' ? 'selected' : ''}>Confirmed</option>
                    <option value="Active" ${booking.status == 'Active' ? 'selected' : ''}>Active (picked up)</option>
                    <option value="Completed" ${booking.status == 'Completed' ? 'selected' : ''}>Completed (returned)</option>
                    <option value="Cancelled" ${booking.status == 'Cancelled' ? 'selected' : ''}>Cancelled</option>
                </select>
            </div>
            <button type="submit" class="btn">${booking != null ? 'Update Booking' : 'Create Booking'}</button>
        </form>
        <a href="bookings" class="back-link">Cancel</a>
    </div>
</body>
</html>